...
@if($layout === 'sidebar')
    ...
    {{-- Doctor Appointments (Doctor Home): doctors only --}}
    @if($role === 'doctor')
...
@else
    ...
    @if(in_array($role, ['admin','supervisor']))
        ...
        {{-- Optionally remove this until you actually have such a route --}}
        {{-- <li>
        <a href="{{ route('roster.management') }}" class="{{ nav_active('roster.management') }}">
            Roster Management
        </a>
        </li> --}}
    @endif
    ...
    {{-- Doctor’s Appointments --}}
    @if($role === 'doctor')
        <li><a href="{{ route('doctor.appointments') }}" class="{{ nav_active('doctor.appointments') }}">Doctor’s Appointments</a></li>
    @endif
